...
function TrophyIconFallback(props) {
  return <Swords {...props} />;
}
  ...
  function renderRightActions(n, progress, dragX) {
    ...
    return (
      <TouchableOpacity style={styles.deleteAction} onPress={() => deleteNotification(n.id)}>
        <Animated.View style={{ transform: [{ scale }] }}><Trash2 size={18} color="#fff" /></Animated.View>
      </TouchableOpacity>
    );
  }
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Swipeable renderRightActions={(progress, dragX) => renderRightActions(item, progress, dragX)} overshootRight={false}>
                <TouchableOpacity style={[styles.row, !item.read && styles.unreadRow]} onPress={clickable ? () => handleTap(item) : undefined} activeOpacity={clickable ? 0.7 : 1}>
                  {!item.read && <View style={styles.unreadDot} />}
                  <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 10 }}>
                    {meta.person ? (
                      <View>
                        <RetryImage source={{ uri: avatarOr(meta.person.avatarUrl, meta.person.id) }} style={styles.avatar} />
                        <View style={[styles.avatarIconBadge, { backgroundColor: meta.color }]}><Icon size={9} color="#fff" /></View>
                      </View>
                    ) : (
                      <View style={[styles.typeIconWrap, { backgroundColor: `${meta.color}26` }]}><Icon size={16} color={meta.color} /></View>
                    )}
                    <View style={{ flex: 1, minWidth: 0 }}>
                      <Text style={styles.text}>{notificationText(item)}</Text>
                      {isPartyInvite && (
                        <View style={styles.actionsRow}>
                          <TouchableOpacity style={styles.acceptBtn} onPress={() => handleRespond(item, true)}><Text style={styles.acceptText}>Kabul Et</Text></TouchableOpacity>
                          <TouchableOpacity style={styles.declineBtn} onPress={() => handleRespond(item, false)}><Text style={styles.declineText}>Reddet</Text></TouchableOpacity>
                        </View>
                      )}
          ...
          ListEmptyComponent={<View style={styles.emptyWrap}><View style={styles.emptyIcon}><Bell size={21} color={c.dim} /></View><Text style={styles.emptyTitle}>Şimdilik sessiz</Text><Text style={styles.empty}>Arkadaşlarından, MatchParty'den, Friend Battle'dan ve sosyal etkileşimlerden gelen gelişmeleri burada göreceksin.</Text></View>}
        />
      )}
    </View>
  );
...
function makeStyles(c) {
  return StyleSheet.create({
    clearAllBtn: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 6, borderRadius: 999, backgroundColor: c.surface2, borderWidth: 1, borderColor: c.border },
    clearAllText: { fontSize: 10.5, fontWeight: "800", color: c.danger },
    listIntro: { marginBottom: 10 }, hint: { fontSize: 10.5, color: c.dim },
    row: { padding: 12, borderRadius: 12, marginBottom: 6, backgroundColor: c.surface, borderWidth: 1, borderColor: c.border, overflow: "hidden" },
    unreadRow: { backgroundColor: c.surface2, borderColor: c.accent }, unreadDot: { position: "absolute", left: 0, top: 0, bottom: 0, width: 3, backgroundColor: c.accent },
    avatar: { width: 36, height: 36, borderRadius: 999, backgroundColor: c.surface2 },
    avatarIconBadge: { position: "absolute", bottom: -3, right: -3, width: 16, height: 16, borderRadius: 999, alignItems: "center", justifyContent: "center", borderWidth: 1.5, borderColor: c.surface },
    typeIconWrap: { width: 36, height: 36, borderRadius: 999, alignItems: "center", justifyContent: "center" },
    moviePoster: { width: 34, height: 49, borderRadius: 6, marginLeft: 4 }, text: { fontSize: 13, color: c.text, lineHeight: 18 },
    emptyWrap: { alignItems: "center", paddingVertical: 56, paddingHorizontal: 28 }, emptyIcon: { width: 48, height: 48, borderRadius: 999, backgroundColor: c.surface2, alignItems: "center", justifyContent: "center", marginBottom: 10 },
    emptyTitle: { color: c.text, fontWeight: "800", fontSize: 14 }, empty: { color: c.dim, fontSize: 11.5, textAlign: "center", lineHeight: 17, marginTop: 5 },
    actionsRow: { flexDirection: "row", gap: 8, marginTop: 9 }, acceptBtn: { flex: 1, backgroundColor: c.accent, borderRadius: 9, paddingVertical: 8, alignItems: "center" }, acceptText: { color: c.bg, fontWeight: "800", fontSize: 12 },
    declineBtn: { flex: 1, backgroundColor: c.surface, borderWidth: 1, borderColor: c.border, borderRadius: 9, paddingVertical: 8, alignItems: "center" }, declineText: { color: c.text, fontWeight: "700", fontSize: 12 },
    deleteAction: { backgroundColor: c.danger, justifyContent: "center", alignItems: "center", width: 64, borderRadius: 12, marginBottom: 6 },
  });
}
