...
const LOGO_DELAY_MS = 3750;
const LOGO_DURATION_MS = 500;
const GLOW_DELAY_MS = 3750;
const GLOW_DURATION_MS = 850;
const TOTAL_MS = 5000;
...
const RINGS = [
  { id: "r1", delay: 300, gradientId: "cA", path: PATH_A, colors: ["#7C3AED", "#C084FC"], from: { x: -190, y: -90, rot: -210, scale: 0.55 }, to: { x: -19, y: -15, rot: -24, scale: 0.85 } },
  { id: "r2", delay: 460, gradientId: "cB", path: PATH_B, colors: ["#DB2777", "#F472B6"], from: { x: 200, y: -55, rot: 230, scale: 0.55 }, to: { x: 22, y: -11, rot: 16, scale: 0.85 } },
  { id: "r3", delay: 620, gradientId: "cC", path: PATH_C, colors: ["#F5A524", "#FCD34D"], from: { x: 180, y: 105, rot: -270, scale: 0.55 }, to: { x: 17, y: 19, rot: -11, scale: 0.85 } },
  { id: "r4", delay: 780, gradientId: "cD", path: PATH_A, colors: ["#22D3EE", "#38BDF8"], from: { x: -45, y: 200, rot: 250, scale: 0.55 }, to: { x: -9, y: 22, rot: 27, scale: 0.85 } },
  { id: "r5", delay: 940, gradientId: "cE", path: PATH_B, colors: ["#3B82F6", "#818CF8"], from: { x: -200, y: 70, rot: -235, scale: 0.55 }, to: { x: -24, y: 7, rot: -16, scale: 0.85 } },
  { id: "r6", delay: 1100, gradientId: "cF", path: PATH_C, colors: ["#A855F7", "#E879F9"], from: { x: 65, y: -205, rot: 275, scale: 0.55 }, to: { x: 11, y: -22, rot: 21, scale: 0.85 } },
];
...
export default function SplashIntroAnim({ onFinish }) {
  ...
  useEffect(() => {
    ambientOpacity.value = withDelay(50, withTiming(1, { duration: 500, easing: Easing.out(Easing.ease) }));

    glowOpacity.value = withDelay(
      GLOW_DELAY_MS,
      withSequence(
        withTiming(1, { duration: GLOW_DURATION_MS * 0.35, easing: Easing.out(Easing.ease) }),
        withTiming(0, { duration: GLOW_DURATION_MS * 0.65, easing: Easing.out(Easing.ease) })
      )
    );
    glowScale.value = withDelay(
      GLOW_DELAY_MS,
      withSequence(
        withTiming(1.15, { duration: GLOW_DURATION_MS * 0.35, easing: Easing.out(Easing.ease) }),
        withTiming(1.6, { duration: GLOW_DURATION_MS * 0.65, easing: Easing.out(Easing.ease) })
      )
    );

    logoOpacity.value = withDelay(LOGO_DELAY_MS, withTiming(1, { duration: LOGO_DURATION_MS, easing: REVEAL_EASING }));
    logoScale.value = withDelay(LOGO_DELAY_MS, withTiming(1, { duration: LOGO_DURATION_MS, easing: REVEAL_EASING }));
